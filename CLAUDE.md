...
src/e1f/
  ...
  performance.py — performance subcommand: EUR valuation, XIRR/TWR/risk metrics; --diff N signed change over N calendar days (ADR-0029); --series N daily cumulative TOTAL over N days (ADR-0030); --metrics portfolio-level extended risk report (incl. days-since-high, best/worst month, trailing 1M/3M/6M), composes with --series; --contrib per-holding Cariño-linked return contribution summing to the TOTAL TWR (ADR-0033)
...
- `ADR-0034` — `glossary` command: a checked-in `data/glossary.md` of every metric (definition + usefulness), read by `e1f glossary` (list all, or substring-match a term like `TWR` / `P&L`); the Markdown file is the single source (parsed, not duplicated in code), name-match short-circuits with a group/body fallback, scoped to stable-command metrics
